...
def preprocess_img(image, input_ksize):
    ...
    image_padded = np.zeros(shape=[nh + pad_h, nw + pad_w, 3], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("./voc2012_512x800_epoch30_loss0.6150.pth", map_location=torch.device('cpu')))
    ...
    for name in names:
        ...
        print("image processed successfully, cost time %.2f ms" % cost_t)
        # print(out)
        ...
        cv2.imwrite("./images_out/" + name, img_pad)