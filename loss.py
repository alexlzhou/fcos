...
class GenTargets(nn.Module):
    ...
    def _gen_level_targets(self, out, gt_boxes, classes, stride, limit_range, sample_radius_ratio=1.5):
        """
        ...
        :param sample_radius_ratio: ?
        :return: cls_targets, ctr_targets, reg_targets
        """
        cls_logits, ctr_logits, reg_preds = out
        ...
        ctr_logits = ctr_logits.permute(0, 2, 3, 1)
        ctr_logits = ctr_logits.reshape((batch_size, -1, 1))
        ...
        assert ctr_targets.shape == (batch_size, h_mul_w, 1)
...
def compute_ctr_loss(preds, targets, mask):
    '''
    Args
...
class LOSS(nn.Module):
    ...
    def forward(self, inputs):
        ...
        cls_logits, ctr_logits, reg_preds = preds
        cls_targets, ctr_targets, reg_targets = targets
        mask_pos = (ctr_targets > -1).squeeze(dim=-1)  # [batch_size,sum(_h*_w)]
        cls_loss = compute_cls_loss(cls_logits, cls_targets, mask_pos).mean()  # []
        cnt_loss = compute_ctr_loss(ctr_logits, ctr_targets, mask_pos).mean()
